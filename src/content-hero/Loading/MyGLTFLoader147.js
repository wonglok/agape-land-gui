  ...
  parse(data, path, onLoad, onError) {
    ...
    if (json.asset === undefined || json.asset.version[0] < 2) {
      if (onError)
        onError(
          new Error(
            'THREE.MyGLTFLoader: Unsupported asset. glTF versions >=2.0 are supported.'
          )
        )
      return
    }
        ...
        switch (extensionName) {
          ...
          default:
            if (
              ...
              console.warn(
                'THREE.MyGLTFLoader: Unknown extension "' + extensionName + '".'
              )
            }
        }
      }
...
class GLTFLightsExtension {
  ...
  _loadLight(lightIndex) {
    ...
    switch (lightDef.type) {
      ...
      default:
        throw new Error(
          'THREE.MyGLTFLoader: Unexpected light type: ' + lightDef.type
        )
    }
  ...
  loadTexture(textureIndex) {
    ...
    if (!loader) {
      if (
        ...
        throw new Error(
          'THREE.MyGLTFLoader: setKTX2Loader must be called before loading KTX2 textures'
        )
      } else {
  ...
  loadTexture(textureIndex) {
    ...
    return this.detectSupport().then(function (isSupported) {
      ...
      if (
        ...
        throw new Error(
          'THREE.MyGLTFLoader: WebP required by asset but unsupported.'
        )
      }
    ...
    if (bufferView.extensions && bufferView.extensions[this.name]) {
      ...
      if (!decoder || !decoder.supported) {
        if (
          ...
          throw new Error(
            'THREE.MyGLTFLoader: setMeshoptDecoder must be called before loading compressed files'
          )
        } else {
...
class GLTFTextureTransformExtension {
  ...
  extendTexture(texture, transform) {
    if (transform.texCoord !== undefined) {
      console.warn(
        'THREE.MyGLTFLoader: Custom UV sets in "' +
          this.name +
...
function assignExtrasToUserData(object, gltfDef) {
  if (gltfDef.extras !== undefined) {
    if (typeof gltfDef.extras === 'object') {
      ...
      console.warn(
        'THREE.MyGLTFLoader: Ignoring primitive type .extras, ' + gltfDef.extras
      )
    }
  }
}
...
function updateMorphTargets(mesh, meshDef) {
  ...
  if (meshDef.extras && Array.isArray(meshDef.extras.targetNames)) {
    ...
    if (mesh.morphTargetInfluences.length === targetNames.length) {
      ...
      console.warn(
        'THREE.MyGLTFLoader: Invalid extras.targetNames length. Ignoring names.'
      )
    }
  }
}
...
function getNormalizedComponentScale(constructor) {
  ...
  switch (constructor) {
    ...
    default:
      throw new Error(
        'THREE.MyGLTFLoader: Unsupported normalized accessor component type.'
      )
  }
}
    ...
    return new Promise(function (resolve, reject) {
      loader.load(
        ...
        function () {
          reject(
            new Error(
              'THREE.MyGLTFLoader: Failed to load buffer "' +
                bufferDef.uri +
                '".'
            )
          )
    ...
    return Promise.all(pendingBufferViews).then(function (bufferViews) {
      ...
      if (accessorDef.sparse !== undefined) {
        ...
        for (let i = 0, il = sparseIndices.length; i < il; i++) {
          ...
          if (itemSize >= 5)
            throw new Error(
              'THREE.MyGLTFLoader: Unsupported itemSize in sparse BufferAttribute.'
            )
        }
      }
  ...
  assignTexture(materialParams, mapName, mapDef, encoding) {
    ...
    return this.getDependency('texture', mapDef.index).then(function (texture) {
      ...
      if (
        ...
        console.warn(
          'THREE.MyGLTFLoader: Custom UV set ' +
            mapDef.texCoord +
            ' for texture ' +
            mapName +
            ' not yet supported.'
  ...
  loadMesh(meshIndex) {
    ...
    return Promise.all(pending).then(function (results) {
          ...
          let onBeforeCompile = (shader, gl) => {
            ...
            let pulse = () => {
              ...
              anime({
                ...
                loop: true,
                begin: () => {
                  // if (window?.follow?.position instanceof Vector3) {
                  // }
                },
              })
            }
            ...
            let rAF = () => {
              ...
              shader.uniforms.resolution.value.x =
                window.innerWidth * window.devicePixelRatio
              shader.uniforms.resolution.value.y =
                window.innerHeight * window.devicePixelRatio

              shader.uniforms.time.value = clock.getElapsedTime()
              shader.uniforms.delta.value = clock.getDelta()

              shader.uniforms.raveColor = { value: new Color('#ffff00') }
      ...
      `
          ...
          throw new Error(
            'THREE.MyGLTFLoader: Primitive mode unsupported: ' + primitive.mode
          )
        }
    ...
    return Promise.all(pending).then(function (results) {
      ...
      for (let i = 0, il = jointNodes.length; i < il; i++) {
        ...
        if (jointNode) {
          ...
          console.warn(
            'THREE.MyGLTFLoader: Joint "%s" could not be found.',
            skinDef.joints[i]
          )
        }
      }
...
function computeBounds(geometry, primitiveDef, parser) {
  ...
  if (attributes.POSITION !== undefined) {
    ...
    if (min !== undefined && max !== undefined) {
      ...
      console.warn(
        'THREE.MyGLTFLoader: Missing min/max properties for accessor POSITION.'
      )
...
function toTrianglesDrawMode(geometry, drawMode) {
  ...
  if (index === null) {
    ...
    if (position !== undefined) {
      ...
      console.error(
        'THREE.MyGLTFLoader.toTrianglesDrawMode(): Undefined position attribute. Processing not possible.'
      )
      return geometry
    }
  }
  ...
  if (newIndices.length / 3 !== numberOfTriangles) {
    console.error(
      'THREE.MyGLTFLoader.toTrianglesDrawMode(): Unable to generate correct amount of triangles.'
    )
  }
