import {
  Color,
  DataTexture,
  FloatType,
  Object3D,
  RGBAFormat,
  Vector3,
  Mesh,
  InstancedBufferAttribute,
  InstancedBufferGeometry,
  IcosahedronBufferGeometry,
  MeshPhysicalMaterial,
  PointLight,
} from 'three'
import { CustomGPU } from './CustomGPU'
import { Core } from '@/content-landing-page/Core/Core'
// import { Trail } from './Trail'
import emitShader from './shader/emitShader.glsl'
import { cNoise } from './NoodleSegmentCompute'

export class PhysicsCompute {
  set shader(v) {
    this._shader = v
    this.setSimShader(v)
  }
  get shader() {
    return this.a
  }
  constructor({ tracker, sizeX, sizeY, gl } = {}) {
    this.sizeX = sizeX
    this.sizeY = sizeY
    this.gl = gl
    //
    this._shader = ''
    this.setSimShader = () => {}
    this.tracker = tracker

    this.core = Core.makeDisposableNode({ name: 'vortex' }).sub

    let SIZE_X = this.sizeX
    let SIZE_Y = this.sizeY

    let gpuCompute = new CustomGPU(SIZE_X, SIZE_Y, gl)
    //
    gpuCompute.setDataType(FloatType)

    // pos IDX
    let posIdx_data = new Float32Array(SIZE_X * SIZE_Y * 4)
    let posIdx = new DataTexture(
      posIdx_data,
      SIZE_X,
      SIZE_Y,
      RGBAFormat,
      FloatType
    )

    //
    let p

    p = 0
    for (let j = 0; j < SIZE_X; j++) {
      for (let i = 0; i < SIZE_Y; i++) {
        let id = p / 4
        posIdx_data[p + 0] = id % 6 // square 1 / 6 index
        posIdx_data[p + 1] = Math.floor(id / 6) // square
        posIdx_data[p + 2] = (SIZE_Y * SIZE_X) / 6.0 // total
        posIdx_data[p + 3] = id
        p += 4
      }
    }

    // pos IDX
    let emitDynamic_data = new Float32Array(SIZE_X * SIZE_Y * 4)
    let emitDynamic = new DataTexture(
      emitDynamic_data,
      SIZE_X,
      SIZE_Y,
      RGBAFormat,
      FloatType
    )

    p = 0
    for (let j = 0; j < SIZE_Y; j++) {
      for (let i = 0; i < SIZE_X; i++) {
        emitDynamic_data[p + 0] = 0
        emitDynamic_data[p + 1] = 0
        emitDynamic_data[p + 2] = 0
        emitDynamic_data[p + 3] = 1.0
        p += 4
      }
    }

    // pos IDX
    let posDynamic_data = new Float32Array(SIZE_X * SIZE_Y * 4)
    let posDynamic = new DataTexture(
      posDynamic_data,
      SIZE_X,
      SIZE_Y,
      RGBAFormat,
      FloatType
    )

    p = 0
    for (let j = 0; j < SIZE_Y; j++) {
      for (let i = 0; i < SIZE_X; i++) {
        posDynamic_data[p + 0] = 0
        posDynamic_data[p + 1] = 0
        posDynamic_data[p + 2] = 0
        posDynamic_data[p + 3] = 1.0
        p += 4
      }
    }

    let vortexShader = /* glsl */ `
#include <common>

precision highp float;
precision highp sampler2D;

uniform vec3 trackerPos;

uniform float iTime;
uniform sampler2D tIdx;
// uniform sampler2D tEmit;

mat3 rotateX(float rad) {
    float c = cos(rad);
    float s = sin(rad);
    return mat3(
        1.0, 0.0, 0.0,
        0.0, c, s,
        0.0, -s, c
    );
}

mat3 rotateY(float rad) {
    float c = cos(rad);
    float s = sin(rad);
    return mat3(
        c, 0.0, -s,
        0.0, 1.0, 0.0,
        s, 0.0, c
    );
}

mat3 rotateZ(float rad) {
    float c = cos(rad);
    float s = sin(rad);
    return mat3(
        c, s, 0.0,
        -s, c, 0.0,
        0.0, 0.0, 1.0
    );
}

mat3 rotateQ (vec3 axis, float rad) {
    float hr = rad / 2.0;
    float s = sin( hr );
    vec4 q = vec4(axis * s, cos( hr ));
    vec3 q2 = q.xyz + q.xyz;
    vec3 qq2 = q.xyz * q2;
    vec2 qx = q.xx * q2.yz;
    float qy = q.y * q2.z;
    vec3 qw = q.w * q2.xyz;

    return mat3(
        1.0 - (qq2.y + qq2.z),  qx.x - qw.z,            qx.y + qw.y,
        qx.x + qw.z,            1.0 - (qq2.x + qq2.z),  qy - qw.x,
        qx.y - qw.y,            qy + qw.x,              1.0 - (qq2.x + qq2.y)
    );
}


#define M_PI 3.1415926535897932384626433832795
float atan2(in float y, in float x) {
  bool xgty = (abs(x) > abs(y));
  return mix(M_PI/2.0 - atan(x,y), atan(y,x), float(xgty));
}
vec3 fromBall(float r, float az, float el) {
  return vec3(
    r * cos(el) * cos(az),
    r * cos(el) * sin(az),
    r * sin(el)
  );
}
void toBall(vec3 pos, out float az, out float el) {
  az = atan2(pos.y, pos.x);
  el = atan2(pos.z, sqrt(pos.x * pos.x + pos.y * pos.y));
}
// float az = 0.0;
// float el = 0.0;
// vec3 noiser = vec3(lastVel);
// toBall(noiser, az, el);
// lastVel.xyz = fromBall(1.0, az, el);


uniform vec3 mousePos;
uniform vec3 screen;
uniform float enterCirlce;
uniform bool isDown;
uniform float tick;

vec3 lerp(vec3 a, vec3 b, float w) {
  return a + w*(b-a);
}

${cNoise()}

void main ()	{
  float time = iTime;
  vec2 cellSize = 1.0 / resolution.xy;
  vec2 newCell = gl_FragCoord.xy;
  vec2 uv = newCell * cellSize;
  vec4 pos = texture2D(tPos, uv);

  vec3 noiser = vec3(
    rand(uv.xy + 0.1) * 2.0 - 1.0,
    rand(uv.xy + 0.2) * 2.0 - 1.0,
    rand(uv.xy + 0.3) * 2.0 - 1.0
  );

  // float az = 0.0;
  // float el = 0.0;
  // toBall(noiser * vec3(10.0, 10.0, 10.0), az, el);
  // az += cos(time * 3.1415) * sin(time * 3.1415);
  // el += sin(time * 3.1415) * cos(time * 3.1415);
  // ball = ball * rotateY(time * 0.5);
  // vec4 yoyo = texture2D(tPos, uv);

  vec4 emitD = texture2D(tEmit, uv.xy);

  if (floor(gl_FragCoord.y) == floor(0.0)) {

    if (isDown){
      gl_FragColor.rgb = 3.0 * vec3(
        rand(time + uv.xy + 0.1) * 2.0 - 1.0,
        rand(time + uv.xy + 0.2) * 2.0 - 1.0,
        rand(time + uv.xy + 0.3) * 2.0 - 1.0
      );
    }

  } else {
    gl_FragColor.rgb = 0.0 * emitD.xyz;
  }



  gl_FragColor.w = 1.0;
}
`

    let posVar = gpuCompute.addVariable('tPos', vortexShader, posDynamic)

    posVar.material.uniforms.tIdx = { value: posIdx }
    posVar.material.uniforms.iTime = { value: 0 }

    this.core.onLoop((dt) => {
      posVar.material.uniforms.iTime.value += dt
    })

    posVar.material.uniforms.mousePos = { value: new Vector3(0, 0, 0) }
    posVar.material.uniforms.screen = {
      value: new Vector3(window.innerWidth, window.innerHeight, 0.0),
    }
    posVar.material.uniforms.enterCirlce = { value: 0 }
    posVar.material.uniforms.trackerPos = { value: this.tracker.position }
    posVar.material.uniforms.normalTrail = { value: null }

    let emitVar = gpuCompute.addVariable('tEmit', emitShader, emitDynamic)
    emitVar.material.uniforms.time = { value: 0 }
    emitVar.material.uniforms.tick = { value: 0 }
    emitVar.material.uniforms.dt = { value: 0 }
    emitVar.material.uniforms.trackerPos = { value: this.tracker.position }

    this.core.onLoop((dt) => {
      emitVar.material.uniforms.dt.value = dt
      emitVar.material.uniforms.tick.value += 1
      emitVar.material.uniforms.time.value += dt
    })

    emitVar.material.uniforms.isDown = { value: false }
    posVar.material.uniforms.isDown = { value: false }
    window.addEventListener('keydown', (ev) => {
      if (ev.key === 'f') {
        emitVar.material.uniforms.isDown.value = true
        posVar.material.uniforms.isDown.value = true
      }
    })

    window.addEventListener('keyup', (ev) => {
      if (ev.key === 'f') {
        emitVar.material.uniforms.isDown.value = false
        posVar.material.uniforms.isDown.value = false
      }
    })

    gpuCompute.setVariableDependencies(emitVar, [posVar, emitVar])
    gpuCompute.setVariableDependencies(posVar, [posVar, emitVar])

    let error = gpuCompute.init()
    if (error !== null) {
      console.error(error)
    }

    this.getHeadList = () => {
      return gpuCompute.getCurrentRenderTarget(posVar).texture
    }
    this.getHeadList2 = () => {
      return gpuCompute.getAlternateRenderTarget(posVar).texture
    }

    this.core.onLoop((dt) => {
      gpuCompute.compute()
    })
  }
}

//

//

//
